...
function mmp.createMapFeature(featureName, roomCharacter)
	...
	if type(roomCharacter) ~= "string" then
		mmp.echo("The new room character must be either a string or nil. " .. type(roomCharacter) .. " is not allowed.")
		return
	end
	...
	if not mapFeatures[lowerFeatureName] then
		...
		mmp.echo("Created map feature '" .. featureName .. "' with the room character '" .. roomCharacter .. "'.")
	else
...
function mmp.roomCreateMapFeature(featureName, roomId)
	...
	local mapFeatures = loadMapFeatures()
	if not mapFeatures[lowerFeatureName] then
		mmp.echo("A feature with name '" .. featureName .. "' does not exist. You need to use 'feature create' first.")
		return
	end
