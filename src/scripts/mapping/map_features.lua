...
function mapper.createMapFeature(featureName, roomCharacter)
	if not featureName or featureName == "" then
		mapper.echo("Can't create an empty map feature.")
		return
	end
	if featureName:find("%d") then
		mapper.echo("Map feature names must not contain numbers.")
		return
	end
	roomCharacter = roomCharacter or ""
	if type(roomCharacter) ~= "string" then
		mapper.echo("The new room character must be either a string or nil. " .. type(roomCharacter) .. " is not allowed.")
		return
	end
	...
	if not mapFeatures[lowerFeatureName] then
		...
		mapper.echo("Created map feature '" .. featureName .. "' with the room character '" .. roomCharacter .. "'.")
	else
		mapper.echo("A map feature with the name '" .. featureName .. "' already exists.")
		return
	end
	return true
end

function mapper.listMapFeatures()
	local mapFeatures = loadMapFeatures()
	mapper.echo("This map has the following features:")
...
function mapper.roomCreateMapFeature(featureName, roomId)
	-- checks for the feature name
	if not featureName then
		mapper.echo("Which feature would you like to create?")
		return
	end
	local lowerFeatureName = featureName:lower()
	local mapFeatures = loadMapFeatures()
	if not mapFeatures[lowerFeatureName] then
		mapper.echo("A feature with name '" .. featureName .. "' does not exist. You need to use 'feature create' first.")
		return
	end
	-- checks for the room ID
	if not roomId then
		if not mapper.currentroom then
		...
		if type(roomId) ~= "number" then
			mapper.echo("Need a room ID as number for creating a map feature on a room.")
			return
		end
	end
	if not getRoomName(roomId) then
		mapper.echo("Room number '" .. roomId .. "' does not exist.")
		return
	end
	-- check if feature already exists
	if table.contains(mapper.getRoomMapFeatures(roomId), lowerFeatureName) then
		mapper.echo("Room '" .. roomId .. "' has already map feature '" .. featureName .. "'.")
		return
	end
	-- create map feature in room
	setRoomUserData(roomId, "feature-" .. lowerFeatureName, "true")
	mapper.echo(string.format("Map feature '%s' created in room number '%d'.", featureName, roomId))
	local featureRoomChar = mapFeatures[lowerFeatureName]
	if featureRoomChar ~= "" then
		setRoomChar(roomId, featureRoomChar)
		mapper.echo("The room now carries the room char '" .. featureRoomChar .. "'.")
	end
	return true
end

function mapper.roomDeleteMapFeature(featureName, roomId)
	-- checks for the feature name
	if not featureName then
		mapper.echo("Which feature would you like to delete?")
		return
	end
	local lowerFeatureName = featureName:lower()
	-- checks for the room ID
	if not roomId then
		...
		if type(roomId) ~= "number" then
			mapper.echo("Need a room ID as number for deleting a map feature from a room.")
			return
		end
	end
	if not getRoomName(roomId) then
	...
	local roomMapFeatures = mapper.getRoomMapFeatures(roomId)
	if not table.contains(roomMapFeatures, lowerFeatureName) then
		mapper.echo("Room '" .. roomId .. "' doesn't have map feature '" .. featureName .. "'.")
		return
	end
	-- delete map feature from room
	setRoomUserData(roomId, "feature-" .. lowerFeatureName, "")
	mapper.echo(string.format("Map feature '%s' deleted from room number '%d'.", featureName, roomId))
	-- now update room char if needed.
	-- first update current map features of this room
	roomMapFeatures = mapper.getRoomMapFeatures(roomId)
	local mapFeatures = loadMapFeatures()
	...
	if getRoomChar(roomId) == mapFeatures[lowerFeatureName] and getRoomChar(roomId) ~= "" then
		...
		if otherRoomMapFeature then
			...
			mapper.echo("Using '" .. newRoomChar .. "' as new room character.")
		else
			-- we didn't find a usable room character, delete it.
			setRoomChar(roomId, "")
			mapper.echo("Deleted the current room character.")
		end
	end
	return true
end

function mapper.getRoomMapFeatures(roomId)
	-- checks for the room ID
	if not roomId then
		if not mapper.currentroom then
			mapper.echo("Don't know where we are at the moment.")
		...
		if type(roomId) ~= "number" then
			mapper.echo("Need a room ID as number for getting all map features of a room.")
			return
		end
	end
	if not getRoomName(roomId) then
...
function mapper.deleteMapFeature(featureName)
	if not featureName or featureName == "" then
		mapper.echo("Which map feature would you like to delete?")
		return
	end
	local lowerFeatureName = featureName:lower()
	local mapFeatures = loadMapFeatures()
	if not mapFeatures[lowerFeatureName] then
		mapper.echo("Map feature '" .. featureName .. "' does not exist.")
		return
	end
	local roomsWithFeature = searchRoomUserData("feature-" .. lowerFeatureName, "true")
	for _, roomId in pairs(roomsWithFeature) do
		local deletionResult = mapper.roomDeleteMapFeature(lowerFeatureName, roomId)
	...
	saveMapFeatures(mapFeatures)
	mapper.echo("Deleted map feature '" .. featureName .. "' from map.")
	return true
end

function mapper.getMapFeatures()
