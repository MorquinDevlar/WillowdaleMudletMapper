...
function mmp.retrievechangelog()
	...
	downloadFile(mmp.changelogfile, "https://www.willowdalemud.com/static/resources/changelog")
end
...
function mmp.downloadmapperscript()
	...
	if io.exists(file) then
		...
		if not s then
			mmp.echo("Couldn't delete the old package (located at %s), because of: %s. This might be a problem.", file, m)
		end
	end
